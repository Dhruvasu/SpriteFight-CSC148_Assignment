...
from a2_skills import SorcererAttack, SorcererSpecial, VampireAttack
from a2_skills import VampireSpecial
...
class Vampire(Character):
    ...
    def attack(self) -> None:
        """
        Perform an attack on Vampire's enemy.
        """
        enemy_old_hp = self.enemy.get_hp()
        super().attack()
        enemy_new_hp = self.enemy.get_hp()
        self_new_hp = self.get_hp() + (enemy_old_hp - enemy_new_hp)
        self.set_hp(self_new_hp)

    def special_attack(self) -> None:
        """
        Perform a special attack on Vampire's enemy.
        """
        enemy_old_hp = self.enemy.get_hp()
...
class Sorcerer(Character):
    ...
    skill_decision_tree - The SkillDecisionTree that Sorcerer uses.
    """
    battle_queue: 'BattleQueue'
    playstyle: 'Playstyle'
    ...
    def set_skill_decision_tree(self, tree: 'SkillDecisionTree') -> None:
        """
        Set this Sorcerer's skill_decision_tree to tree
        ...
        >>> bq = BattleQueue()
        >>> c = Sorcerer("s", bq, ManualPlaystyle(bq))
        >>> c.skill_decision_tree
        >>> c.set_skill_decision_tree(tree)
        >>> c.skill_decision_tree.priority
        5
        """
